...
function App() {
  ...
  useEffect(()=>{
    let tombseged=[...tomb]
    for (let i = 0; i < 100; i++) {
        let random=Math.floor(Math.random()*tombseged.length);
        let random1=Math.floor(Math.random()*tombseged.length);
        let csere=tombseged[random];
        tombseged[random]=tombseged[random1];
        tombseged[random1]=csere;
  ...
  return (
    <div className="App">
      ...
      <h2>
        Ha pályaválasztás előtt állsz, érdemes tisztázni, hogy milyen
        képességekkel rendelkezel. A képességek erősen befolyásolják, hogy
        pályádon, egy foglalkozás gyakorlásánál mennyire leszel sikeres. Az
        alábbi kérdőív ebben jelenthet segítséget: kitöltésével tájékozódhatsz a
        képességeidről, melyek jelentik az erősségeidet és melyek azok,
        amelyekben gyengébb vagy.
      </h2>
      <div>
